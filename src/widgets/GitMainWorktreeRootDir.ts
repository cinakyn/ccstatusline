...
export class GitMainWorktreeRootDirWidget implements Widget {
    getDefaultColor(): string { return 'cyan'; }
    getDescription(): string { return 'Shows the main worktree root directory name (the parent worktree when inside a linked worktree)'; }
    getDisplayName(): string { return 'Git Main Worktree Root Dir'; }
    ...
    render(item: WidgetItem, context: RenderContext, _settings: Settings): string | null {
        ...
        const rootDir = this.getMainWorktreeRootDir(context);
        if (!rootDir) {
            return hideNoGit ? null : 'no git';
        }
    ...
    /**
     * Resolve the main repository's working directory.  `git rev-parse
     * --git-common-dir` returns the shared `.git` directory for the main
     * repository regardless of whether the current directory is a linked
     * worktree; the working tree is its parent.  When the path does not
     * end in `.git` (e.g. a bare or unusual layout) we fall back to the
     * common-dir path itself as the best available identifier.
     */
    private getMainWorktreeRootDir(context: RenderContext): string | null {
        const commonDir = runGit('rev-parse --path-format=absolute --git-common-dir', context);
        if (!commonDir)
            return null;
        const trimmed = commonDir.replace(/[\\/]+$/, '');
        const segments = trimmed.split(/[\\/]/);
        const last = segments[segments.length - 1] ?? '';
        if (last === '.git' || last === '.git.') {
            return segments.slice(0, -1).join('/') || trimmed;
        }
        return trimmed;
    }
