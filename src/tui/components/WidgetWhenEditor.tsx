...
export const WidgetWhenEditor: React.FC<WidgetWhenEditorProps> = ({ widget, onUpdate, onBack, breadcrumbPrefix }) => {
    ...
    const rules: readonly WhenRule[] = widget.when ?? [];
    ...
    function handleListInput(input: string, key: InkKey) {
        ...
        if (key.upArrow) {
            setListIndex(Math.max(0, listIndex - 1));
            return;
        }
        if (key.downArrow) {
            setListIndex(Math.min(Math.max(0, rules.length - 1), listIndex + 1));
            return;
        }
        if (input === 'a' || input === 'A') {
            enterAdd();
    ...
    function handleMoveModeInput(key: InkKey) {
        ...
        if (rules.length < 2)
            return;
        if (key.upArrow && listIndex > 0) {
            const next = [...rules];
            const curr = next[listIndex];
            const prev = next[listIndex - 1];
            if (curr && prev) {
                next[listIndex - 1] = curr;
                next[listIndex] = prev;
        ...
        if (key.downArrow && listIndex < rules.length - 1) {
            const next = [...rules];
            const curr = next[listIndex];
            const below = next[listIndex + 1];
            if (curr && below) {
                next[listIndex] = below;
                next[listIndex + 1] = curr;
                commitWidget({ ...widget, when: next });
                setListIndex(listIndex + 1);
            }
    ...
    function handleTagPickerInput(input: string, key: InkKey) {
        ...
        if (key.delete || input === 'd' || input === 'D') {
            ...
            setDeleteTagState({ tagName: target, refCount });
            setMode('deleteTag');
            return;
        }
        if (input && /^[1-9]$/.test(input)) {
                ...
                <Box marginTop={1}>
                    <ConfirmDialog
                        ...
                        onConfirm={() => {
                            ...
                            setDeleteTagState(null);
                            setPickerIndex(0);
                            // Return to tag picker so user can continue binding.
                            setMode('tag');
                        }}
                        onCancel={() => {
                            setDeleteTagState(null);
            ...
            {mode === 'list' && (
                <>
                    {moveMode ? (
                        ...
                        <Text dimColor>↑↓ select, Enter to move, (a)dd, (e)dit, (d)elete, ESC back</Text>
                    )}
                    <Text dimColor>Priority: rule 1 is highest — first match wins per field.</Text>
                    <Box marginTop={1} flexDirection='column'>
                        {rules.length === 0 ? (
                            <Text dimColor>No conditional actions. Press 'a' to add one.</Text>
                        ) : (
                            rules.map((rule, i) => {
