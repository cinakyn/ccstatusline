...
function buildDynamicEntries(): WhenPredicateEntry[] {
    ...
    for (const manifestEntry of WIDGET_MANIFEST) {
        ...
        if (!widget || !isStatefulWidget(widget))
            continue;
        // Stateful widgets' dynamic state predicates group under the widget's
        // own display name so they don't leak into (and collide with) the
        // static `Core` / `Git` / etc. categories. This keeps the Core category
        // uniform across every widget's when-editor picker.
        const widgetName = widget.getDisplayName();
        for (const state of widget.getAllStates()) {
            entries.push(makeStatefulEntry(manifestEntry.type, state, widgetName, widgetName));
        }
    }
