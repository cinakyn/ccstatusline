...
describe('groups powerline render: groupGap is plain text', () => {
    ...
    it('gap string is also plain when using settings.defaultGroupGap', () => {
        ...
        const result = renderLineEntry(line, { settings: { defaultGroupGap } });

        const gapIndex = result.indexOf(defaultGroupGap);
        expect(gapIndex).toBeGreaterThan(-1);
        expect(result.substring(gapIndex, gapIndex + defaultGroupGap.length)).toBe(defaultGroupGap);
    });
});
