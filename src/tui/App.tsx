...
/**
 * Compute the screen to navigate to after the user picks a line. When
 * groupsEnabled is true we enter the group editor first; otherwise we jump
 * straight to the items editor. The group cursor is always reset to 0 so a
 * stale index from a previous (potentially longer) line can never index
 * into a shorter line's groups[].
 */
export function getLineSelectTransition(groupsEnabled: boolean): {
    nextScreen: Extract<AppScreen, 'groups' | 'items'>;
    nextSelectedGroup: number;
} {
    return {
        nextScreen: groupsEnabled ? 'groups' : 'items',
        nextSelectedGroup: 0
    };
}
...
export const App: React.FC = () => {
    ...
    const handleLineSelect = (lineIndex: number) => {
        const { nextScreen, nextSelectedGroup } = getLineSelectTransition(settings.groupsEnabled);
                ...
                {screen === 'colors' && (
                    <ColorMenu
                        ...
                        onUpdate={(updatedWidgets) => {
                            // ColorMenu receives widgets flattened across ALL
                            // groups via lineWidgets(), so write-back must
                            // preserve each group's widget count by id.
                            // writeFlatWidgets would drop widgets from
                            // groups[1..N] into groups[0] and duplicate them.
                            const newLines = [...settings.lines];
                            newLines[selectedLine] = writeAllGroupsWidgets(newLines[selectedLine], updatedWidgets);
                            setSettings({ ...settings, lines: newLines });
                        }}
                        onBack={() => {
