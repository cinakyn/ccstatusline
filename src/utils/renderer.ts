...
// Shared render environment resolved once per line so flat and grouped paths
// read the same config surface identically.
interface PowerlineRenderEnv {
...
function resolvePowerlineRenderEnv(settings: Settings, context: RenderContext): PowerlineRenderEnv {
    ...
    const lineStartCap = startCaps.length > 0 ? (startCaps[lineIndex % startCaps.length] ?? '') : '';
    const lineEndCap = endCaps.length > 0 ? (endCaps[lineIndex % endCaps.length] ?? '') : '';
    ...
    if (themeName && themeName !== 'custom') {
        ...
        if (theme) {
            const key = colorLevel === 'ansi16' ? '1' : colorLevel === 'ansi256' ? '2' : '3';
    ...
    const autoAlign = Boolean(config.autoAlign);
...
function renderPowerlineElements(
    ...
    for (let i = 0; i < elements.length; i++) {
        ...
        if (isPreserveColors) {
            ...
            // Always reset bold after the widget content so bold never leaks
            // into the separator or cap that follows. In ansi16 terminals,
            // `\x1b[1m` renders foreground colors as their bright variants, so
            // carrying bold through a separator/cap would paint them in a
            // different shade than the widget's own background. If the next
            // widget needs bold, it will re-emit `\x1b[1m` at its own start.
            if (shouldBold)
        ...
        if (needsSeparator) {
            ...
            // The separator belongs to the CURRENT widget's visual pill (its
            // trailing edge). Its foreground tracks the widget's own background
            // so the widget's cap, pill, and right-separator share one colour.
            // When the widget has no bg but the next one does, use reverse-video
            // (`\x1b[7m`) to swap the logical fg=next.bg / bg=terminal-default
            // into a visual (fg=terminal-default, bg=next.bg) pair — producing
            // a fade-in triangle from the transparent terminal bg into the next
            // pill. When neither has a bg, the separator is plain.
            if (shouldInvert) {
                if (widget.bgColor && nextWidget.bgColor) {
                    const fgCode = getColorAnsiCode(bgToFg(sameBackground ? widget.bgColor : nextWidget.bgColor), colorLevel, false);
                    ...
                    separatorOutput = '\x1b[7m' + fgCode + '\x1b[49m' + separator + '\x1b[27m\x1b[39m';
                } else {
...
function renderPowerlineCap(
    ...
    // Adjacent widget has no bg (default) — the pill doesn't exist on this
    // side, so rendering a cap glyph in terminal-default colour would look
    // wrong. Emit a space of the same visual width to preserve layout while
    // making the cap invisible.
    return ' '.repeat(Math.max(1, getVisibleWidth(cap)));
}
 ...
 *
 * Cap resolution: `powerline.startCaps` / `endCaps` cycle by line index; every
 * group on a given line receives the same cap. There is no separate "line-level"
 * cap — groups are visually self-contained pills.
 *
 * Gap resolution: `group.gap ?? settings.defaultGroupGap` placed BEFORE every
 * group except the first visible one.
 *
...
function renderGroupedPowerlineStatusLine(
    ...
    const env = resolvePowerlineRenderEnv(settings, context);
    const {
        themeColors,
        colorLevel,
        terminalWidth,
        ...
        separators,
        invertBgs,
        lineStartCap: groupStartCap,
        lineEndCap: groupEndCap
    } = env;
    const defaultGap = settings.defaultGroupGap;
    ...
    let runningColorIndex = initialColorIndex;
    let widgetOffset = 0;
    ...
    for (let gi = 0; gi < line.groups.length; gi++) {
        ...
        if (!group.continuousColor)
            runningColorIndex = initialColorIndex;
    ...
    if (groupedMaxWidths) {
        ...
        for (const gd of groupData) {
            ...
            const sepWidth = getVisibleWidth(separators[0] ?? '');
    ...
    if (terminalWidth) {
        ...
        for (let vi = 1; vi < visibleGroupData.length; vi++) {
            ...
            const actualGap = gapGroup?.gap ?? defaultGap;
            fixedWidth += getVisibleWidth(actualGap);
        }

        const capsWidth = getVisibleWidth(groupStartCap) + getVisibleWidth(groupEndCap);
        for (const gd of visibleGroupData) {
            let naturalWidth = capsWidth;
            for (let i = 0; i < gd.elements.length; i++) {
                const el = gd.elements[i];
                if (!el)
                    continue;
                ...
                if (next && !el.widget.merge && separators.length > 0) {
                    const sep = separators[Math.min(i, separators.length - 1)] ?? '';
    ...
    for (const gd of visibleGroupData) {
        const { elements, flexCount, sourceGroupIndex } = gd;
        ...
        if (firstVisible) {
            ...
            renderedGroupGaps.push(gapGroup?.gap ?? defaultGap);
        }
        ...
        // Leading cap adjacent to first element's bg
        ...
        if (flexPositions.length === 0 || flexWidth === 0) {
            ...
            // Flex path: split elements by flex-separator positions and
            // interleave spacer runs between the parts.
        ...
        // Trailing cap adjacent to last element's bg. renderPowerlineElements
        // already emitted bold reset per-widget, and chalk.reset below catches
        // any remaining state, so no extra `\x1b[22m` is needed here.
        if (groupEndCap) {
    ...
    // -----------------------------------------------------------------------
    // Assemble final result: visible groups joined by their leading gaps.
    // There is no separate "line-level" cap — each group carries its own cap.
    // -----------------------------------------------------------------------
    let result = '';
...
function renderPowerlineStatusLine(
    ...
    // End cap (adjacent to last widget's bgColor). Bold is already reset
    // inside renderPowerlineElements so the cap is drawn without bold.
    if (endCap) {
    ...
    for (const line of allLines) {
        ...
        for (const widget of lineWidgets(line)) {
            ...
            if (!widgetImpl) {
                ...
                // widget index (e.g. renderGroupedStatusLine) rely on this invariant.
                preRenderedLine.push({
                    content: '',
                    plainLength: 0,
                    widget,
...
/**
 * Render a multi-group line in plain mode by rendering each group separately
 * then joining with the group gap string (raw, no ANSI, no color).
 *
 * Called only when `settings.groupsEnabled === true` AND `line.groups.length > 1`.
 * The `widgets` and `preRenderedWidgets` arrays are the flat concatenation of all
 * group widgets (as produced by `lineWidgets`), so we slice them per-group using
 * the group widget counts.
 *
 * Gap placement: `group.gap ?? settings.defaultGroupGap` is emitted as-is before
 * each group except the first. The gap is raw text (no ANSI wrapping) so it does
 * not inherit surrounding colors or powerline styling.
 *
 * B4: If every widget in a group evaluates to `hidden === true` (or the group has
 * zero widgets), the entire group is dropped — including its preceding gap.
 * An empty group (`widgets: []`) is also treated as hidden (nothing to render).
 *
 * Flex policy (differs from the powerline path): each group is rendered through
 * the flat `renderStatusLine`, which scopes flex-separator budget and truncation
 * to that group alone. The powerline multi-group path instead computes one
 * line-global flex budget and splits it across groups. This asymmetry is
 * intentional — plain mode has no cross-group visual boundary to coordinate,
 * so per-group budgets keep each group self-contained; powerline mode shares
 * a continuous visual baseline across groups, where a line-global budget
 * produces more balanced expansion.
 */
function renderGroupedStatusLine(
    line: Line,
    widgets: WidgetItem[],
    settings: Settings,
    context: RenderContext,
    ...
    let offset = 0;
    // Parallel arrays: one entry per VISIBLE (non-hidden) group.
    const renderedGroups: string[] = [];
    const renderedGroupGaps: string[] = []; // gap BEFORE each visible group

    let isFirstVisible = true;
    for (const group of line.groups) {
        const count = group.widgets.length;
        const groupWidgets = widgets.slice(offset, offset + count);
        const groupPreRendered = preRenderedWidgets.slice(offset, offset + count);
        offset += count;

        // B4: skip fully-hidden groups (including empty-widget groups)
        if (isGroupHidden(groupPreRendered))
            continue;
        ...
        if (isFirstVisible) {
            ...
            renderedGroupGaps.push(group.gap ?? settings.defaultGroupGap);
        }

        // Render this group using the flat renderer (handles colors, separators,
        // inheritSeparatorColors, flex-separator, truncation per-group).
        // We do NOT pass `line` here — single-group call uses the flat path.
        const groupResult = renderStatusLine(
            groupWidgets,
            settings,
            context,
            groupPreRendered,
    ...
    // Join visible groups with their leading gap strings.
    // The gap is plain text (no ANSI codes, no color) so it does not bleed
    // across group color boundaries.
    const parts: string[] = [];
    for (let i = 0; i < renderedGroups.length; i++) {
        const gap = renderedGroupGaps[i] ?? '';
        parts.push(gap);
        const rendered = renderedGroups[i];
...
export function renderStatusLine(
    ...
    // multi-group path is used (powerline vs plain).
    ...
    // When groupsEnabled and the line has multiple groups, render each group
    // independently and join them with per-group gap/cap strings.
    //
    // For powerline mode, renderPowerlineStatusLine handles the multi-group
    // dispatch internally (renderGroupedPowerlineStatusLine). For plain mode,
    // renderGroupedStatusLine handles it.
    //
    // Invariant: groupsEnabled: false OR single-group-per-line → runs the flat
    // path below, producing output byte-identical to the pre-Stage-A render.
    if (settings.groupsEnabled && line && line.groups.length > 1) {
        if (isPowerlineMode) {
            // Delegate directly to the powerline renderer which handles the
            // multi-group path via renderGroupedPowerlineStatusLine.
        ...
        return renderGroupedStatusLine(line, widgets, settings, context, preRenderedWidgets, preCalculatedMaxWidths);
    }
