...
describe('getLineSelectTransition', () => {
    it('routes to the groups screen when groupsEnabled is true', () => {
        const { nextScreen } = getLineSelectTransition(true);
        expect(nextScreen).toBe('groups');
    });

    it('routes to the items screen when groupsEnabled is false', () => {
        const { nextScreen } = getLineSelectTransition(false);
    ...
    it('always resets the group cursor to 0 so a stale index does not leak across line changes', () => {
        ...
        expect(getLineSelectTransition(true).nextSelectedGroup).toBe(0);
        expect(getLineSelectTransition(false).nextSelectedGroup).toBe(0);
    });
});
